...
func TestLoad_Defaults(t *testing.T) {
	...
	if cfg.Ollama.GeneratePath != config.DefaultGeneratePath {
		t.Errorf("GeneratePath: got %q, want %q", cfg.Ollama.BaseURL, config.DefaultGeneratePath)
	}
	...
	if cfg.Ollama.MaxResponseBytes != config.DefaultOllamaMaxResponseBytes {
		t.Errorf("MaxResponseBytes: got %q, want %q", cfg.Ollama.BaseURL, config.DefaultOllamaMaxResponseBytes)
	}
	...
	if cfg.Service.RetryBaseDelay() != config.DefaultRetryBaseDelay {
		t.Errorf("RetryBaseDelaySec: got %q, want %q", cfg.Ollama.BaseURL, config.DefaultRetryBaseDelay)
	}
...
func TestLoad_FromFile(t *testing.T) {
	...
	if got, want := cfg.Ollama.GeneratePath, "/api/generate"; got != want {
		t.Errorf("GeneratePath: got %q, want %q", got, want)
	}
	...
	if got, want := cfg.Ollama.MaxResponseBytes, int64(10485760); got != want {
		t.Errorf("MaxResponseBytes: got %v, want %v", got, want)
	}
	...
	if got, want := cfg.Service.RetryBaseDelay(), time.Second; got != want {
		t.Errorf("RetryBaseDelay: got %d, want %d", got, want)
	}
...
func TestLoad_EnvOverridesFile(t *testing.T) {
	...
	t.Setenv("DEVMATE_CHUNK_THRESHOLD", "9999")
...
func TestLoad_InvalidIntEnvVar_ReturnsError(t *testing.T) {
	...
	cases := []struct {
		...
		{"DEVMATE_REQUEST_TIMEOUT_SEC", "two"},
		{"DEVMATE_CHUNK_THRESHOLD", "large"},
		{"DEVMATE_MAX_CONCURRENCY", "many"},
		{"DEVMATE_MAX_RETRIES", "three"},
		{"DEVMATE_RETRY_BASE_DELAY_SEC", "slow"},
	}
...
// requireNoPanic runs fn and fails the test if it panics.
